...
dir_settings="${BASH_SOURCE[0]%/*}/settings"
for fn in ${dir_settings}/template-*.json; do
    bn=$(basename -- $fn);
    template_name=$(echo $bn|sed -r 's/template\-//'|sed -r 's/\.json$//');

    load_template "$template_name" "$(<"$fn")"
done
